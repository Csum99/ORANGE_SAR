...
| `<XMIN> <XMAX> <XRES>` | X grid, meters — compute in step 4 | `-4 4 0.02` |
| `<YMIN> <YMAX> <YRES>` | Y grid, meters — compute in step 4 | `-4 4 0.02` |
...
**Grid values (`<XMIN> <XMAX> <XRES> <YMIN> <YMAX> <YRES>`) are numbers you pick,
not filenames.** They define the ground patch to image, in meters. Set them so
both reflectors sit comfortably inside the box, with a little margin.

Get the reflector positions first:

```bash
python3 -c "
import pickle, numpy as np
d = pickle.load(open('<RUN_NAME>.pkl','rb'))
# reflectors live in the mocap pickle from step 2:
m = pickle.load(open('../mocap/<MOCAP_PKL>','rb'))
print('Cal_left  (ref_obj1):', np.array(m['ref_obj1']).round(2))
print('Cal_right (ref_obj2):', np.array(m['ref_obj2']).round(2))
"
```

Each prints as `[X  Y  Z]`. Build the grid to span both, plus ~1 m margin:
- `<XMIN>` = (smallest reflector X) − 1   ·   `<XMAX>` = (largest reflector X) + 1
- `<YMIN>` = (smallest reflector Y) − 1   ·   `<YMAX>` = (largest reflector Y) + 1
- `<XRES>` / `<YRES>` = pixel size in meters. Start `0.02`. Use `0.05` for a fast
  first look, `0.01` for a sharper final (slower).

**Worked example.** If `ref_obj1 = [0.95, 1.18, ...]` and `ref_obj2 = [-2.36, -1.87, ...]`:
X spans −2.36→0.95, Y spans −1.87→1.18 → grid `-4 2 0.02 -3 3 0.02` (rounded out with margin).
When unsure, a safe catch-all is `-4 4 0.02 -4 4 0.02`.
